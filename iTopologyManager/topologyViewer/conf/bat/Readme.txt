...
Will be good (but not a must) if the station that is used for running the discovery is able to resolve node hostname through DNS.
...
Then go to the Discovery menu item and click on the Resource tab. The resource settings will be used by the node discovery process.
...
You will have to enter the correct SNMP community settings for your particular node types.
