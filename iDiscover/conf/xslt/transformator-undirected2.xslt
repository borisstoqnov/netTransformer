...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:functx="http://www.functx.com">
	...
	<xsl:template match="/">
		<graphml>
			<graph edgedefault="undirected">
				...
				<xsl:variable name="nodeIDs" select="distinct-values($root//object[objectType='Discovery Interface']/object[objectType='Discovered Neighbor']/name)"/>
				...
				<xsl:variable name="edges">
					<xsl:for-each select="$root//object[objectType='Discovery Interface']">
						...
						<xsl:variable name="remoteInterfaces">
							<xsl:value-of select="distinct-values($root//object[name=$interface]/object[objectType='Discovered Neighbor']/parameters/parameter[name='Neighbor Port']/value)"/>
						</xsl:variable>
							...
							<xsl:when test="$cdp_check='YES' or $lldp_check = 'YES'">
								<xsl:for-each select="object[objectType='Discovered Neighbor']/name">
									<xsl:call-template name="neighbor">
										...
										<xsl:with-param name="remoteInterface" select="../parameters/parameter[name='Neighbor Port']/value"/>
										<xsl:with-param name="nodeID" select="$nodeID"/>
										<xsl:with-param name="neighID" select="."/>
										<xsl:with-param name="localIP" select="../../object[objectType='IPv4 Address']/parameters/parameter[name='IPv4Address']/value"/>
										<xsl:with-param name="remoteIP" select="../parameters/parameter[name='Neighbor IP Address']/value"/>
									...
									<xsl:otherwise>
                                        ...
                                        <xsl:for-each select="$root//object[name=$interface]/object[objectType='Discovered Neighbor' and parameters/parameter[name='Discovery Method' and (contains(value, 'NEXT') or contains(value,'c_'))]]/name">
													...
													<xsl:call-template name="neighbor">
														...
														<xsl:with-param name="nodeID" select="$nodeID"/>
														<xsl:with-param name="neighID" select="$neighID"/>
														<xsl:with-param name="localIP"><xsl:value-of select="../../object[objectType='IPv4 Address']/parameters/parameter[name='IPv4Address']/value"/></xsl:with-param> 
														<xsl:with-param name="remoteIP" select="../parameters/parameter[name='Neighbor IP Address']/value"/>
														<xsl:with-param name="methods" select="$methods_all"/>
													</xsl:call-template>
												</xsl:for-each>
											...
											<xsl:otherwise>
												<xsl:for-each select="$root//object[name=$interface]/object[objectType='Discovered Neighbor' and parameters/parameter[name='Discovery Method'  and contains(value,'MAC')]]/name">
													<xsl:variable name="neighID">
														<xsl:value-of select="."/>
													</xsl:variable>
													<xsl:call-template name="neighbor">
	...
	<xsl:template name="neighbor">
		...
		<edge>
			<xsl:attribute name="id"><xsl:choose><xsl:when test="$localInterface!='' and $remoteInterface!=''"><xsl:variable name="sort2"><root><test><node><xsl:value-of select="$localInterface"/></node></test><test><node><xsl:value-of select="$remoteInterface"/></node></test></root></xsl:variable><xsl:apply-templates select="$sort//root/test"><xsl:sort select="node"/></xsl:apply-templates>-<xsl:apply-templates select="$sort2//root/test"><xsl:sort select="node"/></xsl:apply-templates></xsl:when><xsl:otherwise><xsl:choose><xsl:when test="$localIP!='' and $remoteIP!=''"><xsl:variable name="sort2"><root><test><node><xsl:value-of select="$localIP"/></node></test><test><node><xsl:value-of select="$remoteIP"/></node></test></root></xsl:variable><xsl:apply-templates select="$sort//root/test"><xsl:sort select="node"/></xsl:apply-templates>-<xsl:apply-templates select="$sort2//root/test"><xsl:sort select="node"/></xsl:apply-templates></xsl:when><xsl:otherwise><xsl:apply-templates select="$sort//root/test"><xsl:sort select="node"/></xsl:apply-templates></xsl:otherwise></xsl:choose></xsl:otherwise></xsl:choose></xsl:attribute>
